...
const FarmCards: React.FC = () => {
  ...
  const swappIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'SWAPP',
  )

  const swappPrice =
    swappIndex >= 0 && stakedValue[swappIndex]
      ? stakedValue[swappIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2372500)
  const SWAPP_PER_BLOCK = new BigNumber(60)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [swapp, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'SWAPP' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
